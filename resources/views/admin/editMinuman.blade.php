...
<div class="max-w-2xl mx-auto py-10 px-4">
    ...
    <div class="flex items-center justify-between mb-8 pb-4 border-b border-gray-100">
        <div>
            <h1 class="text-3xl font-extrabold text-gray-800 font-title">Edit Minuman</h1>
            <p class="text-sm text-gray-500 mt-0.5">Ubah rincian menu minuman terpilih</p>
        </div>
    ...
    <!-- Form -->
    <form method="POST" action="{{ route('admin.minumanupdate', $minuman->id) }}" enctype="multipart/form-data" class="bg-white rounded-3xl border border-gray-100 p-8 shadow-sm space-y-6">
        @csrf
        @method('PUT')

        <!-- Name -->
        <div class="space-y-1.5">
            <label for="nama_minuman" class="block text-xs font-bold text-gray-500 uppercase tracking-wider font-semibold">Nama Minuman</label>
            <input type="text" name="nama_minuman" id="nama_minuman" value="{{ old('nama_minuman', $minuman->nama_minuman) }}" required
        ...
        <!-- Description -->
        <div class="space-y-1.5">
            <label for="deskripsi" class="block text-xs font-bold text-gray-500 uppercase tracking-wider font-semibold font-semibold">Deskripsi Lengkap</label>
            <textarea name="deskripsi" id="deskripsi" rows="4" required
                class="w-full p-4 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-gray-50/50 text-sm transition leading-relaxed">{{ old('deskripsi', $minuman->deskripsi) }}</textarea>
        </div>
        ...
        <div class="grid sm:grid-cols-2 gap-6">
            <!-- Category selection -->
            <div class="space-y-1.5">
                <label for="kategori" class="block text-xs font-bold text-gray-500 uppercase tracking-wider font-semibold">Kategori Minuman</label>
                <div class="relative">
                    <select name="kategori" id="kategori" required 
                        ...
                        <option value="Dingin" @if(old('kategori', $minuman->kategori) == 'Dingin') selected @endif>Dingin</option>
                        <option value="Kopi" @if(old('kategori', $minuman->kategori) == 'Kopi') selected @endif>Kopi</option>
                        <option value="Hangat" @if(old('kategori', $minuman->kategori) == 'Hangat') selected @endif>Hangat</option>
                    </select>
            ...
            <div class="space-y-1.5">
                ...
                <div class="relative">
                    ...
                    <input type="number" name="price" id="price" value="{{ old('price', (int) $minuman->price) }}" min="0" required
        ...
        <div class="space-y-1.5">
            ...
            <div class="relative">
                <select name="is_available" id="is_available" required 
                    class="w-full p-4 pr-10 border border-gray-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-amber-500 focus:border-amber-500 bg-gray-50/50 text-sm transition appearance-none">
                    <option value="1" {{ $minuman->is_available ? 'selected' : '' }}>Tersedia (Ready)</option>
                    <option value="0" {{ !$minuman->is_available ? 'selected' : '' }}>Tidak Tersedia (Sold Out)</option>
                </select>
                <span class="material-icons absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-400 pointer-events-none">expand_more</span>
            </div>
        </div>
        ...
        <!-- Image Upload -->
        <div class="space-y-1.5">
            <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider font-semibold font-semibold">Foto Hidangan</label>
            <div class="border-2 border-dashed border-gray-200 rounded-2xl p-6 hover:bg-gray-50 transition flex flex-col items-center justify-center text-center relative group">
                <span class="material-icons text-4xl text-gray-300 group-hover:text-blue-500 transition mb-2">cloud_upload</span>
        ...
        <div class="pt-6 border-t border-gray-100 flex justify-end gap-3">
            ...
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white px-8 py-3 rounded-full text-xs font-bold shadow-md hover:shadow-lg transition">Simpan Perubahan</button>
        </div>
    </form>
</div>
